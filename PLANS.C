...
Ya=Explose[nb].Y[cont]+Explose[nb].StartY;
	if((Ya<200)&&(Ya>0)&&(Xa>0)&&(Xa<TX))   //si le débris est dans le plan...
...
/*for(i=nb;i<longueur;i++)
Page[Y[j+y]+i+x]=coul2;  */
}
}
else
{
...
/*for(i=nb;i<longueur;i++)
Page[Y[j+y]+longueur-i-1+x]=coul2;   */
}
}
