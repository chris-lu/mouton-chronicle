...
for(cont=0;cont<Options.Nb_J_Tot;cont++)
	...
	if(!Mouton[cont].Mort)
		...
		if((temp<25&&temp>-25)&&(temp2<25&&temp2>-25))
			...
			if(dommages<1)
				dommages=1;
			stat1=Mouton[Cur_Joueur].PosX-Bombe.PosX+4;
			stat2=Mouton[Cur_Joueur].PosY-Bombe.PosY+4;
			stat4=sqrt(stat2*stat2+stat1*stat1);
			stat3=(unsigned long)stat4*dommages;
			if (stat3>Stats.Precis)
				{
				Stats.Precis=stat3;
				Stats.Mouton[1]=Cur_Joueur;
				}
			stat3=stat4*25/dommages;
				if (stat3<Stats.Bourrin)
				{
				Stats.Bourrin=stat3;
				Stats.Mouton[2]=Cur_Joueur;
				}
			if((Mouton[cont].Pts_Vie-=dommages)<1)
				{
				if(Cur_Joueur!=cont)
					Mouton[Cur_Joueur].Frags++;
				else
					Mouton[Cur_Joueur].Frags--;
				Par[Cur_Par][0]=cont;
				Par[Cur_Par][1]=random(4)+5;
				Par[Cur_Par++][2]=500;
				if(Cur_Par>3)
					{
					Txt_Down();
			...
			Mouton[cont].Can_Move=0;
			if(temp)
				{
				Mouton[cont].DirX=(dommages/12)*(temp/abs(temp));
				}
			if(temp2)
...
if(!touch)
	{
	Par[Cur_Par][0]=Cur_Joueur;
	Par[Cur_Par][1]=random(4)+1;
	Par[Cur_Par++][2]=400;
	if(Cur_Par>3)
		{
		Cur_Par=3;
...
/*	//Mouton[nb].DirX[cont]=-Mouton[nb].DirX[cont]/2;
	...
	if(Mouton[nb].DirY<-3)
		...
		if((Mouton[nb].Pts_Vie+=Mouton[nb].DirY)<1)
			...
			Mouton[nb].Frags--;
			Mouton[nb].Mort=1;
			Init_Explose(Mouton[nb].PosX,Mouton[nb].PosY+4,CurBomb,64);//creer l'explosion
			CurBomb++;                               //change le numero de la bombe
			CurBomb&=3;
			}
		}
...
if((Xa>x)&&(Xa<(x+320))&&(Ya>200-y)&&(Ya<400-y))  //si dans l'écran...
	Page[(Y[400-y-Ya]+(Xa-x))]=48+Mouton[nb].Equipe;  //...afficher
}
...
while(Mouton[cont].Mort)
	cont++;
last=Mouton[cont].Equipe;
for (cont=0;cont<Options.Nb_J_Tot;cont++)
	if(!Mouton[cont].Mort)
		if(Mouton[cont].Equipe!=last)
				return(0);
return(last+1);
}
...
for(cont=0;cont<Options.Nb_J_Tot;cont++)
	{
	if (Mouton[cont].Frags>Stats.Violant)
		{
		Stats.Mouton[0]=cont;
		Stats.Violant=Mouton[cont].Frags;
		}
	else if (Mouton[cont].Frags<Stats.Inutile)
		{
		Stats.Inutile=Mouton[cont].Frags;
		Stats.Mouton[3]=cont;
		}
	}
}
