...
void Init_Ter(void)
{
unsigned short Ytemp[1280];        //abscisses de chaque point du terrain
...
for (cont=0;cont<TX;cont++)
	{
	for(haut=0;haut<6;haut++)
		Plan_1[cont/320][Y[200-Ytemp[cont]]+(cont%320)+320*haut]=Coul+haut;                      //degradé

	/*	Plan_1[cont/320][temp+320]=17;                  //   "
	Plan_1[cont/320][temp+640]=18;                  //   "
	Plan_1[cont/320][temp+960]=19;                  //   "
	Plan_1[cont/320][temp+1280]=20;                 //   "
	Plan_1[cont/320][temp+1600]=21;                 //   "*/
	for(haut=Ytemp[cont]-6;haut>0;haut--)
		Plan_1[cont/320][Y[200-haut]+(cont%320)]=22+random(2);  //remplissage du terrain
	}
}

												 //ajouter poiter vers fonction
...
for(cont=0;cont<Nb_Spr;cont++)
	GetBlk1(Deco_Inf[cont].X,Deco_Inf[cont].Y,Deco_Inf[cont].Lg,Deco_Inf[cont].Ht,Plan_1[0],Deco_Spr[cont]);
...
/*if(!Options.Ciel||(Options.Ciel==1))
	...
	GetBlk2(1,26,46,46,Page,TrSpr2);
	GetBlk2(25,0,24,24,Page,TrSpr1);
	Inc_Scr(Pv[5],Page);                         //réctifie les couleurs pour la nouvelle pallette
	GetBlk2(0,0,24,24,Page,Lune);
	}
else  if(Options.Ciel==2) */
	{
	sprintf(temp,"Mouton/%s",Back[2]);
	LoadPCX(temp,Page,Pal[1]);       //charger les sprites de fond
	Create_Pal(0,16,Pal[1],Pv[5],Pal[0]);        //modifie la pallette principale
	GetBlk1(1,26,46,46,Page,TrSpr2);
	GetBlk1(25,0,24,24,Page,TrSpr1);
	Inc_Scr(Pv[5],Page);                         //réctifie les couleurs pour la nouvelle pallette
	GetBlk1(0,0,24,24,Page,Lune);
	}

//******************************************
LoadPCX("Mouton/sprites.pcx",Page,Pal[1]);       //charger les sprites de fond
...
for(cont2=0;cont2<4;cont2++)
	for(cont=0;cont<8;cont++)
		GetBlk1(1+9*cont,10*cont2,8,8,Page,Mout_Spr[cont+8*cont2]);
...
Mk_Blood(Pal[0]);
//Mk_Mout_Coul(Pal[0]);
Create_Pal(0,8,PalTemp,16,Pal[0]);        //modifie la pallette principale
Create_Pal(0,12,Team_Coul,244,Pal[0]);        //modifie la pallette principale
